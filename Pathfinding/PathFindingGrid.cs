...
namespace NesScripts.Controls.PathFind
{
    ...
    public class PathFindingGrid
    {
        ...
        ///     < 1.0f = cheap tile.
        /// </param>
        public PathFindingGrid(float[,] tiles_costs)
        {
            // create nodes
            ...
            for (int x = 0; x < gridSizeX; x++)
            {
                ...
                {
                    nodes[x, y] = new PathFindingNode(tiles_costs[x, y], x, y);
                }
            }
        }
        ...
        public PathFindingGrid(bool[,] walkable_tiles)
        {
            // create nodes
            ...
            for (int x = 0; x < gridSizeX; x++)
            {
                for (int y = 0; y < gridSizeY; y++)
                {
                    nodes[x, y] = new PathFindingNode(walkable_tiles[x, y] ? 1.0f : 0.0f, x, y);
                }
            }
        }
        ...
        public System.Collections.IEnumerable GetNeighbours(PathFindingNode node, Pathfinding.DistanceType distanceType)
        {
            int x = 0, y = 0;
            switch (distanceType)
            {
        ...
        PathFindingNode AddNodeNeighbour(int x, int y, PathFindingNode node)
        {
            if (x == 0 && y == 0)
            {
                return null;
